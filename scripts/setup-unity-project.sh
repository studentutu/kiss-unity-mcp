...
PACKAGE_NAME=com.studentutu.kissunitymcp
source_package="$PLUGIN_ROOT/$PACKAGE_NAME"
target_package="$PROJECT/Packages/$PACKAGE_NAME"
target_tools="$PROJECT/.kissunitymcp"
marker="$PROJECT/ProjectSettings/kissunitymcp.json"
# Never install a second copy of the editor types beside the former package.
# Legacy files need an explicit migration; --replace only replaces current paths.
legacy_paths=()
for legacy in "$PROJECT/Packages/com.studentutu.unitysimplemcp" "$PROJECT/.unity-simple-mcp" \
  "$PROJECT/ProjectSettings/SimpleUnityMcpSetup.json" "$PROJECT/.vscode/unity-simple-mcp.code-workspace" \
  "$PROJECT/ProjectSettings/.SimpleUnityMcpRun.lock" "$PROJECT/ProjectSettings/.SimpleUnityMcpSetup.lock"; do
  if [[ -e "$legacy" || -L "$legacy" ]]; then legacy_paths+=("$legacy"); fi
done
...
for target in "$target_package" "$target_tools" "$marker" "$PROJECT/.vscode" "$PROJECT/.vscode/tasks.json" "$PROJECT/.vscode/kissunitymcp.code-workspace"; do
...
stage="$(mktemp -d "${TMPDIR:-/tmp}/kissunitymcp.XXXXXXXX")"
[[ -d "$stage" && "$stage" == "${TMPDIR:-/tmp}"/kissunitymcp.* ]] || fail "Unsafe staging path: $stage"
cleanup() {
  rm -rf -- "$stage"
...
if (( ! dry_run )); then
  ...
  if ! mkdir "$lock" 2>/dev/null; then
    ...
    lock=''
    fail "Setup already running or stale lock: $PROJECT/ProjectSettings/.kissunitymcp-setup.lock. Check processes before removing it."
  fi
fi
...
cp "$PLUGIN_ROOT/templates/kissunitymcp.code-workspace" "$stage/tools/kissunitymcp.code-workspace"
...
if (( ${#legacy_paths[@]} )); then
  state=conflict
  printf 'Legacy installation paths require migration before setup:\n' >&2
  printf '  %s\n' "${legacy_paths[@]}" >&2
fi
report() {
...
(( ${#legacy_paths[@]} == 0 )) || fail 'Move the legacy installation outside the project after reviewing tool settings and closing Unity. See Readme.md. --replace does not migrate legacy paths.'
...
workspace="$PROJECT/.vscode/kissunitymcp.code-workspace"
if [[ -e "$workspace" ]] && ! cmp -s "$workspace" "$stage/tools/kissunitymcp.code-workspace"; then
...
transaction="$PROJECT/Packages/.kissunitymcp-setup-$$"
[[ "$transaction" == "$PROJECT/Packages/".kissunitymcp-setup-* && ! -e "$transaction" ]] || fail "Unsafe transaction path: $transaction"
mkdir "$transaction"
...
if [[ ! -e "$workspace" ]]; then workspace_written=1; cp "$stage/tools/kissunitymcp.code-workspace" "$workspace"; fi
...
if (( package_moved || tools_moved )); then
  ...
  [[ "$transaction" == "$PROJECT/Packages/".kissunitymcp-setup-* && -d "$transaction" && ! -L "$transaction" ]] || fail "Unsafe staging cleanup: $transaction"
  rm -rf -- "$transaction"
fi
